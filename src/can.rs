...
use stm32_hal2::{gpio::Port, pac::interrupt};
...
fn enable_can_xcvr(enable: bool) {
    if enable {
        crate::hal::gpio::set_low(Port::B, 7);
    } else {
        crate::hal::gpio::set_high(Port::B, 7);
    }
}
...
pub fn init_can() {
    ...
    // 500kbit with 8MHz input clock (APB1)
    const PRESC: NonZeroU8 = NonZeroU8::new(1).unwrap();
